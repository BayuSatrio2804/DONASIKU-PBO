...
const DashboardPenerima = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        // Filter available donations - check both possible status locations and values
        const availableDonations = donasiList.filter(d => {
          const status = d.statusDonasi?.status || d.status || '';
          const isAvailable = status.toLowerCase() === 'tersedia' || status.toLowerCase() === 'aktif' || status.toLowerCase() === 'available';
          return isAvailable && d.jumlah > 0;
        });
  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(d => {
        // Backend deskripsi contains "Barang: [nama]. [deskripsi]"
        const desc = d.deskripsi?.toLowerCase() || '';
        // lokasi can be string or object with alamatLengkap
        const lok = typeof d.lokasi === 'string'
          ? d.lokasi.toLowerCase()
          : (d.lokasi?.alamatLengkap?.toLowerCase() || '');
        const kat = d.kategori?.toLowerCase() || '';

        return desc.includes(term) || lok.includes(term) || kat.includes(term);
      });
    }
  ...
  const getStatusBadge = (req) => {
    ...
    if (req.status_permohonan === 'received' || req.status_pengiriman === 'received') {
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-8 py-8">
                      ...
                      src={
                        donation.image.startsWith('http') || donation.image.startsWith('data:')
                          ...
                          : donation.image.startsWith('storage/')
                            ...
                            : `http://localhost:8081/storage/${donation.image}`
                      }
        ...
        {myRequests.filter(req => req.status_permohonan !== 'received' && req.status_permohonan !== 'rejected' && req.status_pengiriman !== 'received').length > 0 && (
          <div className="mb-12 mt-12 pt-8 border-t border-gray-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {myRequests.filter(req => req.status_permohonan !== 'received' && req.status_permohonan !== 'rejected' && req.status_pengiriman !== 'received').map((req) => (
