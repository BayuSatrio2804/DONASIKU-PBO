    ...
    <div className="p-6 flex gap-6 min-h-[60vh]">
      ...
      <section className="flex-1 flex flex-col bg-white rounded-xl shadow">
        ...
        <div className="flex-1 p-4 overflow-auto" ref={listRef} style={{ background: '#F7FAFC' }}>
          ...
          {messages.map((m) => (
            <div key={m.id} className={`mb-3 flex group ${String(m.sender_id) === String(currentUserId) ? 'justify-end' : 'justify-start'}`}>
              <div className={`flex items-end gap-2 max-w-[70%] ${String(m.sender_id) === String(currentUserId) ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`px-4 py-2 rounded-xl ${String(m.sender_id) === String(currentUserId) ? 'bg-[#007EFF] text-white' : 'bg-white text-gray-800 shadow'}`}>
                  <div className="text-sm">{m.message}</div>
                  <div className={`text-xs mt-1 text-right ${String(m.sender_id) === String(currentUserId) ? 'text-blue-100' : 'text-gray-400'}`}>{new Date(m.created_at).toLocaleString('id-ID')}</div>
                </div>
