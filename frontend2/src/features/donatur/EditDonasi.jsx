  ...
  useEffect(() => {
    const fetchDonation = async () => {
      try {
        ...
        // Extract nama from deskripsi if it follows "Barang: xxx. yyy" format
        let nama = '';
        let cleanDeskripsi = donation.deskripsi || '';
        if (cleanDeskripsi.startsWith('Barang: ')) {
          const match = cleanDeskripsi.match(/^Barang: (.+?)\. (.*)$/);
          if (match) {
            nama = match[1];
            cleanDeskripsi = match[2];
          } else {
            nama = cleanDeskripsi.replace('Barang: ', '');
            cleanDeskripsi = '';
          }
        } else {
          nama = cleanDeskripsi; // Fallback: use deskripsi as nama
        }

        // Handle lokasi - can be object or string
        const lokasiStr = typeof donation.lokasi === 'object'
          ? (donation.lokasi?.alamatLengkap || '')
          : (donation.lokasi || '');

        // Handle status - can be in statusDonasi.status or status directly
        const statusStr = donation.statusDonasi?.status || donation.status || 'aktif';

        setFormData({
          nama: nama,
          kategori: donation.kategori || 'pakaian',
          jumlah: donation.jumlah || 1,
