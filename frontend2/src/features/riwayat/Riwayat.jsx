    ...
    const loadHistory = async () => {
      ...
      try {
        ...
        if (role === 'donatur') {
          ...
          const myDonations = allDonasi.filter(d => {
            const donaturId = d.donatur?.userId || d.donatur?.id;
            const status = (d.statusDonasi?.status || d.status || "").toLowerCase();
            return String(donaturId) === String(user.id || user.userId) && (status === 'selesai' || d.jumlah <= 0);
          }).map(d => ({
            ...d,
            nama: extractNamaBarang(d),
            deskripsi: extractDeskripsi(d),
            lokasi: getLokasiString(d.lokasi),
            sourceType: 'donasi_manual',
            donatur_name: user.name, // Self
            updatedAt: d.updated_at,
            displayStatus: 'completed', // Ensure it counts as completed
            image: d.image // Ensure image is passed explicitly
          }));

          // 2. Get fulfilled requests that are received
          const fulfilledHistory = permintaanList.filter(req => {
            const donaturId = req.donatur?.userId || req.donatur?.id;
            return String(donaturId) === String(user.id || user.userId) && (req.status_permohonan === 'received' || req.status_pengiriman === 'received');
          }).map(req => ({
            ...req,
            nama: req.judul,
            jumlah: req.target_jumlah,
            lokasi: getLokasiString(req.lokasi),
            ...
            image: req.donation?.image || req.image // Prioritize donation image
          }));
          ...
          const historyFromPermintaan = permintaanList.filter(
            req => req.status_permohonan === 'received' || req.status_pengiriman === 'received' || req.status_permohonan === 'rejected' || req.status === 'terpenuhi'
          ).map(req => ({
            ...req,
            nama: req.judul,
            jumlah: req.jumlah || req.target_jumlah,
            displayStatus: (
              req.status_permohonan === 'received' ||
              req.status_pengiriman === 'received' ||
              req.status_permohonan === 'approved' ||
              req.status_permohonan === 'sent' ||
              req.status_permohonan === 'Fulfilled' ||
            ...
            // Fix Image: If fulfilled/completed, show DONATION image. Fallback to PROOF image.
            image: req.donation?.image || req.image,
          ...
          const historyFromDetail = detailDonasiList.filter(
            ...
            image: dd.donation?.image, // Direct donations always have donation image
            displayStatus: 'completed',
            updatedAt: dd.updated_at || dd.created_at,
            // Extra details
            deskripsi: extractDeskripsi(dd.donation),
            lokasi: getLokasiString(dd.donation?.lokasi),
              ...
              >
                <div className="relative h-48 overflow-hidden bg-gray-100">
                  ...
                  <div className="absolute top-3 right-3">
                    <span className={`inline-flex items-center space-x-1 px-3 py-1 rounded-full text-sm font-bold shadow-sm ${item.displayStatus === 'completed' ? 'bg-green-100 text-green-700' :
                      ...
                      <span>
                        {item.displayStatus === 'completed' ? 'Selesai' :
                  ...
                  <div className="space-y-3 mb-6 flex-grow">
                    ...
                    {item.lokasi && (
                      <div className="flex items-center space-x-2 text-sm text-gray-600 px-2">
                        ...
                        <span className="line-clamp-1">{getLokasiString(item.lokasi)}</span>
                      </div>
                    )}
                    <div className="flex items-center space-x-2 text-sm text-gray-600 px-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <p className="font-semibold text-gray-800 flex items-center gap-2">
                    ...
                    <span className="truncate">{getLokasiString(selectedItem.lokasi)}</span>
                  </p>
                </div>
              </div>
