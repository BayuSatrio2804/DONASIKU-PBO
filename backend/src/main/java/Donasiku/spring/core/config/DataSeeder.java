...
import Donasiku.spring.core.repository.AdminRepository;
...
public class DataSeeder {
    ...
    CommandLineRunner initDatabase(StatusDonasiRepository statusRepo,
        ...
        return args -> {
            ...
            // Add "Tersedia" status (this is what DonasiService looks for first)
            if (statusRepo.findByStatus("Tersedia").isEmpty()) {
                StatusDonasi sTersedia = new StatusDonasi();
                sTersedia.setStatus("Tersedia");
                sTersedia.setStatusVerifikasi(true);
                statusRepo.save(sTersedia);
            }
            if (statusRepo.findByStatus("Available").isEmpty()) {
                StatusDonasi s1 = new StatusDonasi();
                s1.setStatus("Available");
                s1.setStatusVerifikasi(true);
                statusRepo.save(s1);
            }
            if (statusRepo.findByStatus("Dikirim").isEmpty()) {
                StatusDonasi s2 = new StatusDonasi();
                s2.setStatus("Dikirim");
                s2.setStatusVerifikasi(true);
            ...
            if (statusRepo.findByStatus("Menunggu Konfirmasi").isEmpty()) {
                StatusDonasi s4 = new StatusDonasi();
                s4.setStatus("Menunggu Konfirmasi");
                s4.setStatusVerifikasi(true);
                statusRepo.save(s4);
            }
            if (statusRepo.findByStatus("Diterima").isEmpty()) {
                StatusDonasi s3 = new StatusDonasi();
                s3.setStatus("Diterima");
                s3.setStatusVerifikasi(true);
                statusRepo.save(s3);
            }
            ...
            if (userRepo.findByUsername("admin").isEmpty()) {
                ...
                // TODO: Admin entity creation requires special handling with @MapsId
                // For now, User with admin role is sufficient
                // Admin entity can be created via API endpoint later if needed
            ...
            // p1.setJumlah(20);
            // p1.setDeskripsiKebutuhan("Butuh buku pelajaran untuk siswa kelas 3-5 SD. Buku
            // dalam kondisi baik atau seperti baru.");
            ...
            // p2.setJumlah(50);
            // p2.setDeskripsiKebutuhan("Membutuhkan pakaian untuk anak-anak berusia 5-12
            // tahun. Ukuran S, M, L. Pakaian sudah tidak terpakai.");
            ...
            // p3.setJumlah(100);
            // p3.setDeskripsiKebutuhan("URGENT! Anak-anak kami membutuhkan makanan pokok
            // dan susu untuk kebutuhan sebulan ke depan. Mohon bantuan segera.");
